...
export default function DependentSection({ dependents, onChange }: Props) {
  ...
  const handleAdd = () => {
    ...
    if (!newDependent.dob) {
      ...
      const birthDate = new Date(newDependent.dob);
    ...
    if (!newDependent.emailAddress || !emailRegex.test(newDependent.emailAddress)) {
    ...
    if (newDependent.id && newDependent.dob) {
      const existingDependent = dependents.find(dep => dep.id === newDependent.id);
      if (existingDependent) {
        const currentDOB = new Date(existingDependent.dob);
        const updatedDOB = new Date(newDependent.dob);
  ...
  const handleEmergencySOS = () => {
    const emergencyContact = dependents.find(dep => dep.emergencySosContact);
    if (emergencyContact) {
      alert(`Sending emergency alert to ${emergencyContact.fullName} at ${emergencyContact.emailAddress}`);
    } else {
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewDependent({ ...newDependent, fullName: e.target.value })
                }
                className={`w-full rounded-lg border ${errors.fullName ? 'border-red-500' : 'border-gray-300'} shadow-sm py-3 px-4 focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
  ...
  <input
    ...
    onChange={(e) => {
      ...
      setNewDependent({ ...newDependent, dob: dob });
    ...
    className={`w-full rounded-lg border ${errors.dob ? 'border-red-500' : 'border-gray-300'} shadow-sm py-3 px-4 focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
  />
  {errors.dob && <p className="text-xs text-red-500">{errors.dob}</p>}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  type="email"
                  value={newDependent.emailAddress || ''}
                  onChange={(e) =>
                    setNewDependent({ ...newDependent, emailAddress: e.target.value })
                  }
                  className={`pl-10 w-full rounded-lg border ${errors.emailAddress ? 'border-red-500' : 'border-gray-300'} shadow-sm py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
    ...
    {dependents.map((dependent) => (
      <div
        ...
        className={`mb-6 p-4 border p-4 rounded-lg ${
          dependent.emergencySosContact ? 'border-green-500 bg-green-50' : ''
        }`}
      >
        <div className="flex justify-between items-center">
          <div>
            <p className="font-semibold">{dependent.fullName}</p>
            <p className="text-sm text-gray-500">{dependent.relation}</p>
            <p className="text-sm text-gray-500">Date of Birth: {dependent.dob}</p>
            <p className="text-sm text-gray-500">Phone: {dependent.mobileNumber}</p>
            <p className="text-sm text-gray-500">Email: {dependent.emailAddress}</p>
          </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className={`${
                dependent.emergencySosContact ? 'text-red-500' : 'text-gray-500'
